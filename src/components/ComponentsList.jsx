    ...
    <div className="section">
      <div className="container is-x">
        ...
        <table className="table has-text-centered is-striped is-bordered">
          <thead>
          <tr>
            <th className="is-first"><span className="is-dark">Component Name</span></th>
            {frameworks
              .filter(framework => visible.has(framework.slug))
              .map(framework => {
                return (
                  <th key={framework.slug}><span className="is-dark">{framework.name}</span></th>
                );
              })}
          </tr>
          </thead>
            ...
            .map(component => {
              return (
                <tr key={component.name.toLowerCase()}>
                  <td>
                    <span className="tag is-info">{component.name}</span>
                  </td>
                  ...
                  {frameworks.filter(framework => visible.has(framework.slug)).map(framework => {
                    return (
                      <td key={framework.slug}>
                        ...
                        <a href={component.supports[framework.slug]} target="_blank"
